...
class BaseDictSet(data.Dataset):
    ...
    def __init__(self, data_path, image_list_file, patch_size=None, split='train', load_npy=True, repeat=1,
        ...
        with open(self.image_list_file, 'r') as f:
            for i, img_pair in enumerate(f):
                ...
                need_wb=False
                if len(img_pair.split(' '))==7:
                    need_wb=True
                if need_wb:
                    img_file, lbl_file, iso, focus, wb1, wb2, wb3 = img_pair.split(' ')
                else:
                ...
                # wb=None
                if need_wb:
                    wb1,wb2,wb3=np.float32(wb1),np.float32(wb2),np.float32(wb3)
                    wb=np.array([wb1,wb2,wb3])
                else:
                    wb=np.array([1.,1.,1.])
    ...
    def __getitem__(self, index):
        ...
        if isinstance(info['white_balance'], np.ndarray):
            clean_raw=self._white_balance_adjust(clean_raw, info['white_balance'])
        ...
        return {
            ...
            'ratio': info['ratio'],
            "white_balance": info['white_balance']
        }
...
class SonyDictSet(BaseDictSet):
    ...
    def _white_balance_adjust(self,clean_raw, wb):
        R,G,B=wb[0],wb[1],wb[2]
        wb=np.array([R,G,B,G])
        wb=wb[:,None,None]
        clean_raw=wb*clean_raw
