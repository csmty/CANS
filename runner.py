...
def main(config):
    ...
    for epoch in range(config['train'].get('start_epoch', 0)+1, total_epochs+1):
        ...
        # else:
        #     train_one_epoch_auto(config, train_forward, model, loss_list, train_dataloader, optimizer, scaler, epoch, lr_scheduler, writer)

        if epoch % config['valid_per_epoch'] == 0 or epoch>200:
...
def train_one_epoch_auto(config, train_forward, model, loss_list, data_loader, optimizer, scaler, epoch, lr_scheduler, writer):
    torch.cuda.reset_peak_memory_stats()
    model.train()
    num_steps = len(data_loader)
    batch_time = AverageMeter()
    ...
    for idx, data in tqdm(enumerate(data_loader)):
        ...
        # 使用GradScaler缩放损失，然后进行反向传播
        scaler.scale(loss).backward()

        # 使用GradScaler进行梯度的未缩放回步骤，并更新权重
        scaler.step(optimizer)

        # 更新GradScaler为下一轮训练准备
        scaler.update()
